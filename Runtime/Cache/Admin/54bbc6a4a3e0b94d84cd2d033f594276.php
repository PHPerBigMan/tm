...
<script>
    var vm = new Vue({
        el: 'body',
        data: {
            carousel : <?php echo ($data); ?>
        },
        methods: {
            add: function (id) {
                layer_show("添加广告轮播",'/admin/Carousel/add/',600,500)
            },
            edit: function (id) {
                layer_show("修改广告轮播",'/admin/Carousel/edit/?id='+id,600,500)
            },
            shanchu: function (id,event) {
                layer.msg('确定要删除吗？', {
                    ...
                    btn: ['确定', '取消'],
                    yes: function(){
                        $.get('/admin/Carousel/del/?id='+id,function (data) {
                            $temp = $.parseJSON(data);
                            if($temp['status'] == "200"){
