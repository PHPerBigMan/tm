...
<script>
    ...
    methods: {
        add: function (id) {
            layer_show("新增规格",'/admin/Specifications/add/',500,250)
        },
        edit: function (id) {
            layer_show("编辑规格",'/admin/Specifications/edit/?id='+id,500,250)
        },
        shanchu: function (id,event) {
            layer.msg('确定要删除吗？', {
                ...
                btn: ['确定', '取消'],
                yes: function(){
                    $.get('/admin/Specifications/del/?id='+id,function (data) {
                        $temp = $.parseJSON(data);
                        if($temp['status'] == "200"){
