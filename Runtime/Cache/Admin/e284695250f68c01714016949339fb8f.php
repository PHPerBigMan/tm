    ...
    <form class="am-form am-form-horizontal" action="" method="post" id="from1">
        <div class="am-g am-margin-top">
            ...
            <div class="am-u-sm-8 am-u-md-4">
                <input type="hidden" class="am-input-sm" name="carriageid" value="<?php echo ($data["carriageid"]); ?>">
                <input type="text" class="am-input-sm" name="title" placeholder="请输入规格名称" value="<?php echo ($data["title"]); ?>">
            </div>
            ...
            <div class="am-u-sm-8 am-u-md-8">
                <template v-for="item in province">
                    <label class="am-radio-inline">
                        <template v-if="item.name == '<?php echo ($data["province"]); ?>'">
        ...
        <div class="am-g am-margin-top">
            ...
            <div class="am-u-sm-8 am-u-md-4">
                <input type="text" class="am-input-sm" name="price" placeholder="单元:元(不单独设置价格将采用默认价格)" value="<?php echo ($data["price"]); ?>">
            </div>
            <div class="am-hide-sm-only am-u-md-6"></div>
        </div>
                            ...
                            <tbody>
                                <tr v-for="item in data">
                                    ...
                                    <td>
                                        ...
                                        <input type="text" class="am-input-sm yunfei am-fl" value="{{ item.price }}" name="price2[]">元
                                    </td>
        ...
        <div class="am-g am-margin-top-sm">
            <div class="am-u-sm-9 am-u-sm-offset-2">
                <?php if($data["carriageid"] == '' ): ?><button type="button" class="am-btn am-btn-primary">添加运费模版</button>
                    <?php else: ?>
                    <button type="button" class="am-btn am-btn-primary">修改运费模版</button><?php endif; ?>
            </div>
        </div>
    </form>
...
<script>
    $(".am-btn-primary").click(function () {
        $.post("<?php echo U('carriage/handle');?>",$("#from1").serialize(),function (data) {
            $temp = $.parseJSON(data)
            if($temp['status'] == "200"){
...
<script>
    var vm = new Vue({
        el: 'body',
        data: {
            province: <?php echo (json_encode($province )); ?>,
            data : <?php echo ((isset($extend) && ($extend !== ""))?($extend):"[]"); ?>,
        },
        ready: function () {
            console.log(this)
        },
