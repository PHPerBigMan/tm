...
<head>
    ...
    <link rel="stylesheet" href="<?php echo C('CDN_PATH');?>/Public/resource/font/iconfont.css">
    <style>
...
<div class="page-group">
    <div class="page">
        <nav class="bar bar-tab">
            ...
            <a class="tab-item active" href="<?php echo U('cart/index');?>">
...
<script>
    $(function () {
        $.showIndicator();
        $.post('/Home/cart/index.html',function (data) {
        ...
        methods: {
            del: function (id,event) {
                $.confirm('确定要删除吗?', function () {
                    $.showIndicator();
                    $.get('/Home/Cart/del/?id='+id,function (data) {
                        $temp = $.parseJSON(data);
                        if($temp['status'] == "200"){
            ...
            jia:function(id,catid){
                $.showIndicator();
                $.get('/Home/Cart/change/?type=2&id='+catid,function (data) {
                    $temp = $.parseJSON(data)
                    if($temp['status'] == "200"){
                        vm.data[id]['nubs']++
                    }else{
            ...
            jian:function(id,catid){
                ...
                $.showIndicator();
                $.get('/Home/Cart/change/?type=1&id='+catid,function (data) {
                    $temp = $.parseJSON(data)
                    if($temp['status'] == "200"){
                        vm.data[id]['nubs']--
                    }else{
