...
<head>
    ...
    <link rel="stylesheet" href="<?php echo C('CDN_PAHT');?>/Public/resource/font/iconfont.css?<?php echo C('CDN_VERSION');?>">
    <style>
                    ...
                    $.post('/Home/address/add/?id='+id,$("#address").serialize(),function(data){
                        ...
                        if($temp['status'] == "200"){
                            if(vm.type == 3 || vm.type == 4){
                                ...
                                if(vm.type == 4){
                                    $.get("/Home/address/cale?id=<?php echo $_GET['id'];?>&addressid="+vm.data.address.addressid,function(data){
                                        $temp = $.parseJSON(data);
                                        if($temp['status'] == "404"){
                                            $.toast($temp['text']);
                                            $.hideIndicator();
...
//                                        vm.data.address = vm.address[0];
            ...
            jiesuan:function(){
                ...
                if(vm.beizhu != ""){
                    $.post("/Home/order/beizhu",{id:<?php echo $_GET['id'];?>,beizhu:vm.beizhu},function(data){

                    })
                }
                $.showIndicator();
                $.post("/Home/order/xiadan",{id:<?php echo $_GET['id'];?>,title:vm.data.snop[0].snopjson.title},function(data){
                    $temp = $.parseJSON(data);
                    if($temp['status'] == "404"){
                        $.toast($temp['text']);
                        $.hideIndicator();
            ...
            xuanzhe:function(key,item){
                ...
                $.get("/Home/address/cale?id=<?php echo $_GET['id'];?>&addressid=" + item.addressid,function(data){
                    $temp = $.parseJSON(data);
                    if($temp['status'] == "404"){
                        $.toast($temp['text']);
                        $.hideIndicator();
